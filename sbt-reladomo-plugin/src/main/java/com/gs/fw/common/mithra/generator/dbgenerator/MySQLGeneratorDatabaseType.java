...
public class MySQLGeneratorDatabaseType extends AbstractGeneratorDatabaseType
{
    private static final int MYSQL_MAX_INDEX_NAME = 64;
    ...
    public void generateIdxFile(MithraObjectTypeWrapper wrapper, File outDir) throws IOException
    {
        ...
        for (int i = 0; i < indices.size(); i++)
        {
            ...
            {
                writer.println("alter table " + tableName + " drop index " + getFixedIndexName(index) + ";");
    ...
    private String getFixedIndexName(Index index)
    {
        ...
        if (name.length() > MYSQL_MAX_INDEX_NAME)
        {
            int targetLength = MYSQL_MAX_INDEX_NAME;
            ...
            if (idxIndex > 0)
            {
                ...
                targetLength = MYSQL_MAX_INDEX_NAME - (index.getName().length() - idxIndex);
            }
