...
case class NewTask(name: String, status: String) extends NewTemporalTransactionalObject {
  override lazy val underlying: JavaTask = {
    val underlyingObj = new JavaTask(TimestampUtil.infinityDate())
    underlyingObj.setName(name)
    underlyingObj.setStatus(status)
    underlyingObj
  }
  def insert()(implicit tx: Transaction): Task = {
    underlying.insert()
    Task(underlying)
  }
  def insertForRecovery()(implicit tx: Transaction): Task = {
    underlying.insertForRecovery()
    Task(underlying)
  }
  def cascadeInsert()(implicit tx: Transaction): Task = {
